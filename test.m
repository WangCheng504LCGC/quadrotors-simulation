...
% Last Modified by GUIDE v2.5 07-May-2018 20:23:40
...
set(handles.t2,'string',sprintf('滑动数据： %3.0f',val));
...
% --- Executes on key press with focus on al and none of its controls.
function al_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to al (see GCBO)
...
if get(hObject,'value')
    ...
    if index%如果没有选择取消
        set(handles.t3,'string',[pathname filename])
    end
end
...
% hObject    handle to val_disp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --- Executes on button press in slid_permit.
function slid_permit_Callback(hObject, eventdata, handles)
if get(hObject,'value')==1
    set(handles.slider8,'enable','on')%设置滑动条为激活状态
else
    set(handles.slider8,'enable','off')
end
% hObject    handle to slid_permit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of slid_permit
...
if strcmp(sel,'open')%若双击了鼠标左键
    str=get(hObject,'String');%获取列表框了-的所有选项文本
    n=get(hObject,'value');
    set(handles.sub_sel,'string',str(n));%str{n});
end
...
% --- If Enable == 'on', executes on mouse press in 5 pixel border.
% --- Otherwise, executes on mouse press in 5 pixel border or over sub_sel.
function sub_sel_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to sub_sel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
stra={'哈尔滨';'大庆';'阿城';'齐齐哈尔';'黑河'};
strb={'武汉';'黄冈';'襄樊';'宜昌';'荆州';'孝感'};
switch sel
    case 1 %若当前选中下拉菜单的第一项
        set(handles.city,'string','wait','value',1)
    case 2
